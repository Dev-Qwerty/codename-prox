    ...
    fn(obj) {
      if(this.cchecker == null) {
        ...
        if(this.cchecker == "Customer") {
          ...
          window.location.href = location.protocol + "//"+ location.host + "/services/carpentry/ctmodal"
        }
      }  
    },
    apiCall() {
      let url = this.$serverURLI +'/services/5ea7ddc3a5cad8efe1a49020'
      this.$http.get(url)
      .then((response) => {
