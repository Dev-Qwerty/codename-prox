...
import Phmodal from '@/components/Services/Photography/Phmodal.vue'
    ...
    fn(obj) {
      if(this.cchecker == null) {
        ...
        if(this.cchecker == "Customer") {
          this.$cookies.set("sarray", JSON.stringify(obj), "id");
          window.location.href = location.protocol + "//"+ location.host + "/services/photography/phmodal"
        }
      } 
    },
    apiCall() {
      let url = 'http://localhost:3000/services/5ef8be2fd213605c72b6192c'
      this.$http.get(url)
      .then((response) => {
